...
select order_name, date(date_time) , sum(quantity * (Select pizza_price from pizza_info where orderdetails.pizza_id = pizza_info.pizza_id)) as total 
from orderdetails
left join pizza_info on orderdetails.orderDetail_id = pizza_info.pizza_id
group by order_name, date(date_time);